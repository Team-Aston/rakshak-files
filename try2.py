...
driver_serial = serial.Serial('/dev/ttyS0', 115200, timeout=1)
...
def send_to_ddsm(left_speed, right_speed):
    ...
    driver_serial.write(f'{{"T":1,"ID":1,"cmd":{int(left_speed)},"act":3}}\n'.encode())  # Left motor
    driver_serial.write(f'{{"T":1,"ID":2,"cmd":{int(right_speed)},"act":3}}\n'.encode())  # Right motor
