import serial import argparse import threading import json import time from dronekit import connect from rplidar import RPLidar
...
DIRECTIONS = { 'front': ((330, 360), 0, 25),  # Forward 'right': ((60, 120), 1, 24),   # Right 'back': ((150, 210), 2, 12),   # Back 'left': ((240, 300), 3, 8),    # Left }

def read_ddsm_serial(ddsm_ser): """ Continuously read from the DDSM serial port.""" while True: try: data = ddsm_ser.readline().decode('utf-8', errors='ignore') if data: pass  # Uncomment to debug: print(f"[DDSM] {data}", end='') except Exception as e: print(f"[Error reading DDSM] {e}")

def scale_servo_to_speed(servo_value): """ Maps servo PWM range 1000-2000 µs to -200 to 200 speed units. """ if servo_value is None: return 0 return int((servo_value - 1735) / 500 * 200)

def lidar_loop(port, vehicle): """ Reads RPLIDAR scans, segments into directions, and sends MAVLink DISTANCE_SENSOR messages. """ try: lidar = RPLidar(port) print("[LIDAR] Started scanning...")
...
def main(): parser = argparse.ArgumentParser( description='Bridge Pixhawk servo outputs + RPLIDAR obstacle data to DDSM driver' ) parser.add_argument('ddsm_port', type=str, help='Serial port for DDSM HAT (e.g., /dev/ttyUSB0)') parser.add_argument('--pixhawk', type=str, default='/dev/ttyAMA0', help='MAVLink port for Pixhawk (e.g., /dev/ttyAMA0)') parser.add_argument('--lidar_port', type=str, default='/dev/ttyUSB1', help='Serial port for RPLIDAR (e.g., /dev/ttyUSB1)') args = parser.parse_args()
